...
try:
    ...
    def get_scheduler_status(db_path: str = "nifty100.db") -> dict:
        return {"last_run": None, "total_cached_predictions": 0, "next_scheduled_run": "16:00 IST Next Weekday"}

    def run_daily_market_close_pipeline(force: bool = False, run_type: str = "API", db_path: str = "nifty100.db") -> dict:
...
@router.get("/predict/scheduler/status", summary="Get status of automated daily market-close pipeline")
def get_automation_status():
    """
    Returns status of the automated daily market-close pipeline, last run timestamp,
    total cached predictions, and next scheduled execution time.
    """
    return get_scheduler_status()


@router.post("/predict/scheduler/trigger", summary="Trigger automated daily pipeline immediately")
def trigger_daily_pipeline(force: bool = False):
    """
    Triggers full daily market-close pipeline on demand: downloads latest market prices,
    trains ML models, and saves next-day predictions for all 92 companies.
    """
    try:
        result = run_daily_market_close_pipeline(force=force, run_type="API_TRIGGER")
        return result
    except Exception as e:
